(function () {
  ...
  menuToggle.addEventListener("click", (event) => {
    const target = event.target,
      menuItem = target.classList.contains("menu-accordion__btn")
        ? target.parentNode
        : target.parentNode.parentNode, // родитель (li)
    ...
    if (
      ...
      target.classList.contains(menuButtonSpan)
    ) {
      ...
      if (menuItem.classList.contains(menuToggleActive)) {
        ...
        } else if (menuWidth <= breakPointPhone) {
          // если true, то контент во всю ширину родителя
          menuToggleWidth = menuItemAll[0].querySelector(".menu-accordion__img")
            .clientWidth;
          for (let i = 0; i < menuItemAll.length; i++) {
            if (menuItemAll[i] === menuItem) {
              // нажатая(активная) li
              menuItemAll[i].querySelector(
                ".menu-accordion__content"
              ).style.width = menuWidth - menuToggleWidth + "px";
            } else {
              menuItemAll[i].querySelector(
    ...
    if (menuWidth <= breakPointPhone) {
      for (let i = 0; i < menuItemAll.length; i++) {
        setTimeout(() => {
          ...
          menuItemAll[i].querySelector(".menu-accordion__img").style.width =
            menuToggleWidth + "px";
        }, 300);
      }
    }
